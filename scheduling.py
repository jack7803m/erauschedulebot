...
def check_metadata(inputmetadata):
    ...
    if (
        inputmetadata["format"] != template_metadata["format"]
        or inputmetadata["creator"] != template_metadata["creator"]
        or inputmetadata["trapped"] != template_metadata["trapped"]
        or inputmetadata["encryption"] != template_metadata["encryption"]
    ):
        return False
    else:
        return True
...
def extract_data(filename):
    ...
    table_list = tabula.read_pdf(
        filename, pages="all", output_format="json", lattice=True
    )
    campus = "none"
    ...
    for table in table_list:
        ...
        for lis in table["data"]:
            if i > 0:
                ...
                if campus == "none":
                    if re.match(r".*DB.*", filtered_sectionID) is not None:
                        campus = "daytona"
                    elif re.match(r".*PC.*", filtered_sectionID) is not None:
                        campus = "prescott"
            i += 1
    ...
    extracted = {
        "studentid": studentid,
        "name": name,
        "classes": class_and_section_list,
        "campus": campus,
    }
    return extracted
